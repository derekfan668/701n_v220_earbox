...
enum LOAD_METHOD {
    ...
    LOAD_FROM_SDCARD,				//只有JPEG从SD卡读数才适用
};


//测试项
//<单选项>
#define LARGE_IMAGE_SCALE_OPTIMIZE_TEST     0 //大图片(宽或者高超过240)缩放显示[优化], 注意:这种方式只适用RGB565/RGB888
#define LARGE_IMAGE_SCALE_TEST              0 //大图片(宽或者高超过240)缩放显示
#define IMB_MIXED_TEST                      0 //外部背景数据叠加imb任务
#define IMB_WATCH_TEST                      0 //模拟时钟显示
#define JPEG_SD_TEST 						1//jpeg从sd卡读取数据解码,只能有一个task使用这种从sd卡读取的方式！！！注意，从SD卡读取数据解码，比直接指向地址的方式会慢一些！！
//<可多选>
...
/***************jpeg图片存放在SD卡时使用*************/
struct jpeg_input_info {
    u16 w;
    u16 h;
    void *jpg_fat_info;//必须确保指针指向的是全局的变量地址
...
static struct jpeg_input_info jpg_info = {0};

#define APP_SD_ROOT 	"storage/sd0/C/"

#define K_LEN			(4096)


/* static u8 * g_jpg_dbuf = NULL; */
/* static u32 g_jpg_size = 0; */

/***************jpeg图片存放在SD卡时使用*************/
//该函数由用户实现
//用于通过SD卡文件系统打开一张jpeg图片
//返回获取的文件句柄
static void *jpg_open_fat_info(char *path)
{
    /* return g_jpg_dbuf; */
    ...
    if (hdl == NULL) {
        ...
        printf("sdcard_fopen succ %x\n", (u32)hdl);
    }

    return hdl;
}
//该函数由用户实现
//用于通过SD卡文件系统关闭一张jpeg图片
static void jpg_close_fat_info(struct jpeg_input_info *info)
{
    /* if (info->jpg_fat_info) { */
    /* info->jpg_fat_info = NULL; */
    /* } */

    //以下是示例
    if (info->jpg_fat_info) {
        fclose(info->jpg_fat_info);
        info->jpg_fat_info = NULL;
    }
}
//该函数由用户实现,传递给jpeg模块内部的回调函数,
//注意：jpeg内部调用该回调时，可能会连续两次传递同一个offset_4k的偏移值，用户可以使用类似
//下面示例的方式记录上一次的offset_4k(info->last_offset_4k = info->offset_4k),在重复第二次
//获取相同偏移的数据时，直接返回，不需要重复SD卡读来获取，加快读取速度!!!
//
//SD卡方式下用于发送信号通知后台读取jpeg内容
//fat_info:文件句柄
//offset_4k:读取的偏移，4K字节为单位
//len:读取的实际长度
static u8 *jpg_from_sd(void *fat_info, u32 *len, u32 offset_4k)
{
    //以下是示例
    struct jpeg_input_info *info = (struct jpeg_input_info *)fat_info;
    /* printf("post_read %x\n", (u32)info); */
    /* int kstart = jiffies_msec(); */

    info->offset_4k = offset_4k;//通知后台要读取的偏移

    /* printf("offset %d\n", offset_4k); */

    if ((info->last_offset_4k == offset_4k) && info->len && info->buf_4k) {
        *len = info->len;//读取的长度
        return info->buf_4k;
    }
    info->buf_ok = 0;
    ...
    os_sem_post(&info->sem);//发信号通知后台

    /* printf("v1\n"); */
    while (!info->buf_ok); //等待读取完成
    /* printf("v2\n"); */

    *len = info->len;//读取的长度
    /* printf("post_read_end %x, %x\n", *len, (u32)info->buf_4k); */

    if (info->len == 0) {
        info->last_offset_4k = -1;
        printf("get jpg_src_data err\n");
        return NULL;
    }

    info->last_offset_4k = info->offset_4k;

    /* printf("get time : %d ms\n", jiffies_msec() - kstart); */
    return info->buf_4k;
}
//该函数由用户实现
//用于通过SD卡文件系统读取jpeg内容
static void jpg_from_sd_task(struct jpeg_input_info *info)
{
    //以下是示例
    int ret;
    int offset = info->offset_4k * K_LEN;
    /* printf("offset_4k %d\n", info->offset_4k); */

    /* int rlen; */
    /* u8 * raddr; */
    /* info->len = 0; */
    /* if (offset >= g_jpg_size) { */
    /* return; */
    /* } */
    /* rlen = g_jpg_size - offset; */
    /* rlen = (rlen > 4096) ? 4096 : rlen; */
    /* raddr = g_jpg_dbuf + offset; */
    /* memcpy(info->buf_4k, raddr, rlen); */
    /* info->len = rlen; */

    /* int kstart = jiffies_msec(); */
    /* printf("c1 %x, %x\n", offset, flen(info->jpg_fat_info)); */
    info->len = 0;
    if (offset >= flen(info->jpg_fat_info)) {
        printf("sdcard offset overflow err %d, %d", offset, flen(info->jpg_fat_info));
        return ;
    }
    /* printf("c2"); */
    /* printf("g1\n"); */
    ret = fseek(info->jpg_fat_info, offset, SEEK_SET);
    /* if (ret != offset) { */
    if (ret != 0) {
    ...
    /* printf("g2\n"); */
    /* printf("c3 %x, %x\n", (u32)info->jpg_fat_info, (u32)info->buf_4k); */
    ret = fread(info->jpg_fat_info, info->buf_4k, K_LEN);
    /* printf("g3\n"); */
    /* printf("c4 %x\n", ret); */
    if ((ret > 0) && (ret <= K_LEN)) {
        info->len = ret;
...
/***************jpeg图片存放在SD卡时使用*************/
static void jpg_sd_task(void *arg)
{
    struct jpeg_input_info *info = (struct jpeg_input_info *)arg;

    /* printf("sd_jpeg_task\n"); */
    while (1) {
        os_sem_pend(&info->sem, 0);
        /* printf("once\n"); */
        jpg_from_sd_task(info);
...
/***************jpeg图片存放在SD卡时使用*************/
//每次换新的Jpeg图片时，都要调用一次该函数
static int jpg_get_info(struct jpeg_input_info *info, int lcd_format, char *path)
{
    ...
    if (flag == 0) {
        os_sem_create(&info->sem, 0);

        task_create(jpg_sd_task, info, "jpg_sd");
    ...
    /* printf("a1 %x\n", (u32)info); */
    if (info->buf_4k == NULL) {
        info->buf_4k = imb_malloc(K_LEN, true);
        if (info->buf_4k == NULL) {
            return -1;
        }
        /* printf("buf_4k %x\n", (u32)info->buf_4k); */
    ...
    info->jpg_fat_info = jpg_open_fat_info(path);
    if (info->jpg_fat_info == NULL) {
        printf("jpg openfile err\n");
        return -1;
    }
    info->jpg_ginfo_cb = jpg_from_sd;

    //这里的目的只是为了提前获取Jpg的宽高，用于后面的配置
    //如果用户知道jpeg图片的宽高，就直接赋值,不需要解码
    imb_task_lock();
    ret = jpeg_dec_init(info, lcd_format, info->jpg_ginfo_cb);
    imb_task_unlock();
    if (ret != 0) {
    ...
    info->w = jpg_fd->info.x;//jpeg图片本身的宽度
    info->h = jpg_fd->info.y;//jpeg图片本身的高度
    /* printf("a3 %d, %d\n", info->w, info->h); */
...
/***************jpeg图片存放在SD卡时使用*************/
//合成完调用一次，里面会关闭文件句柄和释放相关资源
static void jpg_free_info(struct jpeg_input_info *info)
{
    os_sem_set(&info->sem, 0);
    info->w = 0;
    info->h = 0;
    info->buf_ok = 0;

    jpg_close_fat_info(info);
    if (info->buf_4k) {
        imb_free(info->buf_4k);
        info->buf_4k = NULL;
    }
    printf("free_jpg_sd\n");
}
...
void imb_create_jpeg_task(int task_id, struct image_file *file, u32 priv, int lcd_format, int x, int y, int width, int height, int load_method)
{
    struct imb_task_info imb_tsk_info = {0};
    struct imb_task *p = NULL;
    struct jpeg_input_info *jpg_info = NULL;

    imb_tsk_info.elm_id = task_id;
    if (load_method == LOAD_FROM_FLASH_WITH_TAB) {
        imb_tsk_info.data_src = DATA_SRC_FLASH;//直接从flash取数
        imb_tsk_info.cur_in_flash = 1;//数据存在flash
    } else if (load_method == LOAD_FROM_SDCARD) {
        jpg_info = (struct jpeg_input_info *)file;
        imb_tsk_info.data_src = DATA_SRC_SRAM;
        imb_tsk_info.cur_in_flash = 0;
    } else {
        imb_tsk_info.data_src = DATA_SRC_SRAM;
    ...
    if (jpg_info) {
        /***************jpeg图片存放在SD卡时使用*************/
        imb_tsk_info.in_format      = PIXEL_FMT_JPEG;
        imb_tsk_info.x_offset       = x;
        imb_tsk_info.y_offset       = y;
        imb_tsk_info.src_w          = jpg_info->w;
        imb_tsk_info.src_h          = jpg_info->h;
        imb_tsk_info.des_w          = width;
        imb_tsk_info.des_h          = height;
        imb_tsk_info.zip_en         = 0;
        ...
        imb_tsk_info.priv           = 0;
        imb_tsk_info.jpg_ginfo_cb   = jpg_info->jpg_ginfo_cb;
        /* imb_tsk_info.jpg_fat_info  = jpg_info->jpg_fat_info; */
        imb_tsk_info.jpg_fat_info  = jpg_info;//必须确保指针指向的是全局的变量地址
    } else {
        imb_tsk_info.in_format      = file->format;
        imb_tsk_info.x_offset       = x;
        imb_tsk_info.y_offset       = y;
...
//只能有一个JPEG控件使用这种从sd卡读取的方式！！！
//jpeg从sd卡读取数据解码,只能有一个task使用这种从sd卡读取的方式！！！
//注意，从SD卡读取数据解码，比直接指向地址的方式会慢一些！！
void imb_jpeg_test_sdcard()
{
    int i;
    char *str_tab[] = {"storage/sd0/C/test0.jpg", "storage/sd0/C/test1.jpg", "storage/sd0/C/test2.jpg"};
    ...
    printf("begin_jpg_sd\n");
    //注意，假如有多张图片，但只在一个地方显示，
    //相当于只有一个控件显示图片,那么这个task_id
    //只需要获取一次，只创建一个任务的意思
    static int my_task_id = -1;
    if (my_task_id == -1) {
        my_task_id = imb_task_get_id();
    }
    ...
    while (1) {
        ...
        /* jpg_get_info(&jpg_info, lcd_info.color_format, APP_SD_ROOT"test0.jpg"); */
        printf("jpg_root: %d, %s\n", i, str_tab[i]);
        jpg_get_info(&jpg_info, lcd_info.color_format, str_tab[i]);

        imb_create_jpeg_task(my_task_id, &jpg_info, 0, lcd_info.color_format, 0, 0, jpg_info.w, jpg_info.h, LOAD_FROM_SDCARD);
        ...
        /* printf("jpeg time : %d ms\n", jiffies_msec() - kstart); */

        //合成后记住要释放！！！
        jpg_free_info(&jpg_info);
...
#error "TCFG_PSRAM_DEV_ENABLE Need Enable"
    ...
    /* int id = PAGE0_e4c4_TEST0; */
    /* ret = open_image_by_id(0, res->ui_resfile, &file, id & 0xff, id >> 16); */
    /* ASSERT(!ret); */

    /* u32 addr = get_image_addr(res->ui_resfile, RES_PATH"ui/ui.res", id);//获取jpeg图片的flash物理地址 */

    /* printf("jpeg_buf %d, %d, %d\n", file.width, file.height, file.len); */
    /* put_buf(addr, 64); */
    /* g_jpg_dbuf = addr; */
    /* g_jpg_size = file.len; */

    imb_jpeg_test_sdcard();
#else
