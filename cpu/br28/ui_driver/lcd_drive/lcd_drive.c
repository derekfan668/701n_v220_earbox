...
static void lcd_reset(struct lcd_drive *lcd)
{
    if (lcd->reset) {
        ...
        if(gpio_read(lcd_dat->pin_reset)==0){
            gpio_direction_output(lcd_dat->pin_reset, 1);
            os_time_dly(10);            
        }
        printf("gpio_direction_output(lcd_dat->pin_reset, 0);");
...
int lcd_sleep_ctrl(u8 enter)
{
    ...
    if (enter) {
        ...
        extern  u8 alipay_check_open_status;
        if (alipay_check_open_status) {
            extern void alipay_power_off(void);
