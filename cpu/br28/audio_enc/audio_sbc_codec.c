...
#define SBC_ABANDON_START_MS	500 //开始时静音时间
...
#if SBC_ABANDON_START_MS
    int abandon_start_size;
#endif /* #if SBC_ABANDON_START_MS */
...
#if SBC_ABANDON_START_MS
        if (sbc_enc->abandon_start_size) {
            if (pcm_len) {
                if (sbc_enc->abandon_start_size < pcm_len) {
                    sbc_enc->abandon_start_size = 0;
                    sbc_enc_pcm_data_fade_in(enc->pcm_frame, pcm_len);
                    /* putchar('B'); */
                } else {
                    sbc_enc->abandon_start_size -= pcm_len;
...
static int sbc_enc_output_handler(struct audio_encoder *encoder, u8 *frame, int len)
{
    ...
    // printf("w:%d,%d,%d\n", wlen, len, cbuf_get_data_size(&sbc_enc->output_cbuf));
    if (wlen != len) {
        putchar('F');
        return len;
    }
...
int a2dp_sbc_encoder_get_data(u8 *packet, u16 buf_len, int *frame_size)
{
    ...
    *frame_size = debug_frame_size;
    ...
    // printf("r:%d, %d\n",rlen, cbuf_get_data_size(&sbc_enc->output_cbuf));
    if (rlen == 0) {
        putchar('N');
    }
...
int audio_sbc_enc_open()
{
    ...
    fmt.priv = &sbc_param;
#if PC_AUDIO_EMITTER
    fmt.sample_rate = audio_sbc_enc_get_rate();
...
#if SBC_ABANDON_START_MS
    sbc_enc->abandon_start_size = audio_sbc_enc_get_rate() * audio_sbc_enc_get_channel_num() * 2 * SBC_ABANDON_START_MS / 1000;
    printf("abandon start size :%d \n", sbc_enc->abandon_start_size);
#endif /* #if SBC_ABANDON_START_MS */
