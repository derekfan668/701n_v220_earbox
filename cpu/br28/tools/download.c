...
if [ -f version ]; then
    files="app.bin version $1.elf p11_code.bin br28loader.bin br28loader.uart uboot.boot uboot.boot_debug ota.bin nor_ota.bin ota_debug.bin flash_params.bin isd_config.ini isd_download.exe ufw_maker.exe fw_add.exe"
else
    files="app.bin $1.elf p11_code.bin br28loader.bin br28loader.uart uboot.boot uboot.boot_debug ota.bin nor_ota.bin ota_debug.bin flash_params.bin isd_config.ini isd_download.exe fat_image_tool.exe ufw_maker.exe fw_add.exe"
...
#if (defined(CONFIG_BOARD_7014F5_DEMO))
call download/watch/download_insert_7014N.bat
#elif (defined(TCFG_VIRFAT_INSERT_FLASH_ENABLE) && (TCFG_VIRFAT_INSERT_FLASH_ENABLE))
call download/watch/download_insert.bat
/* #elif (defined(TCFG_NANDFLASH_DEV_ENABLE) && (TCFG_NANDFLASH_DEV_ENABLE)) */
#elif (defined CONFIG_BOARD_701N_NANDFLASH_DEMO)	// 使用nandflash板级
call download/watch/download_nandflash.bat
#else
