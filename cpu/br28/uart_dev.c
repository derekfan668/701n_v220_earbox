...
struct uart_deepsleep_arg {
    JL_UART_TypeDef *uart;
    uart_bus_t *uart_bus;
    u32 baud : 24;
};

static struct uart_deepsleep_arg uart_dp_arg[3] = {0};
...
const uart_bus_t *uart_dev_open(const struct uart_platform_data_t *arg)
{
    ...
    if (CONFIG_UART0_ENABLE && uart_is_idle(0)) {
        ut_num = 0;
    } else if (CONFIG_UART1_ENABLE && uart_is_idle(1)) {
        ut_num = 1;
    } else if (CONFIG_UART2_ENABLE && uart_is_idle(2)) {
        ut_num = 2;
    } else {
        return NULL;
    }
    ...
    if (CONFIG_UART0_ENABLE && ut_num == 0) {
        ...
        ut = &uart0;
        uart_dp_arg[0].uart_bus = ut;
        uart_dp_arg[0].baud = arg->baud;
        uart_dp_arg[0].uart = JL_UART0;
        ...
        UT1_open(arg->baud, arg->is_9bit,
                 ...
                 arg->frame_length, arg->rx_timeout);
        ut = &uart1;
        uart_dp_arg[1].uart_bus = ut;
        uart_dp_arg[1].baud = arg->baud;
        uart_dp_arg[1].uart = JL_UART1;
        ...
        UT2_open(arg->baud, arg->is_9bit,
                 ...
                 arg->frame_length, arg->rx_timeout);
        ut = &uart2;
        uart_dp_arg[2].uart_bus = ut;
        uart_dp_arg[2].baud = arg->baud;
        uart_dp_arg[2].uart = JL_UART2;
    } else {
        return NULL;
    }
    return ut;
...
u32 uart_dev_close(const struct uart_platform_data_t *arg, uart_bus_t *ut)
{
    ...
    if (CONFIG_UART0_ENABLE && (&uart0 == ut)) {
        uart_dp_arg[0].uart = NULL;
        UT0_close();
        uart_config_free(arg, PFI_UART0_RX);
        return gpio_close_uart0();
    } else if (CONFIG_UART1_ENABLE && (&uart1 == ut)) {
        uart_dp_arg[1].uart = NULL;
        UT1_close();
        uart_config_free(arg, PFI_UART1_RX);
        return gpio_close_uart1();
    } else if (CONFIG_UART2_ENABLE && (&uart2 == ut)) {
        uart_dp_arg[2].uart = NULL;
        UT2_close();
...
{
    struct uart_deepsleep_arg *uart_arg;
    JL_UART_TypeDef *uart;
    uart_bus_t *uart_bus;

    for (int i = 0; i < 3; i++) {
        uart_arg = &uart_dp_arg[i];
        uart = uart_arg->uart;
        uart_bus = uart_arg->uart_bus;

        if (!uart) {
            continue;
        }

        uart->BAUD	 = ((UART_CLK + uart_arg->baud / 2) / uart_arg->baud) / 4 - 1;
        if (uart_bus->kfifo.buffer) {
            if (uart_bus->rx_timeout > 10) {
                uart->OTCNT = (uart_bus->rx_timeout / 10) * (UART_OT_CLK / 10) / 10;
            } else {
                uart->OTCNT = uart_bus->rx_timeout * UART_OT_CLK / 1000;
            }
        }
    }
}
CLOCK_CRITICAL_HANDLE_REG(uart_dev, clock_critical_enter, clock_critical_exit)
...
static JL_UART_POWEROFF UART_POWEROFF[3];

AT_VOLATILE_RAM_CODE_POWER
static u8 uart_dev_enter_deepsleep(void)
{
    struct uart_deepsleep_arg *uart_arg;
    JL_UART_TypeDef *uart;
    ...
    for (int i = 0; i < 3; i++) {
        ...
        /* UART_POWEROFF[i].BAUD = ((UART_CLK + uart_arg->baud / 2) / uart_arg->baud) / 4 - 1; */
        UART_POWEROFF[i].CON0 =  uart->CON0;
        UART_POWEROFF[i].CON1 =  uart->CON1;
        /* UART_POWEROFF[i].OTCNT =  uart_clk / 100; */
        UART_POWEROFF[i].TXADR =  uart->TXADR;
        /*UART_POWEROFF[i].RXSADR =  uart->RXSADR;
        UART_POWEROFF[i].RXEADR =  uart->RXEADR;*/
        UART_POWEROFF[i].RXCNT =  0xffff;
        UART_POWEROFF[i].CON2 =  uart->CON2;
    }
    return 0;
}

AT_VOLATILE_RAM_CODE_POWER
static u8 uart_dev_exit_deepsleep(void)
{
    struct uart_deepsleep_arg *uart_arg;
    JL_UART_TypeDef *uart;
    uart_bus_t *uart_bus;
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (uart_bus->kfifo.buffer) {
            uart_bus->kfifo.buf_in = 0;
            uart_bus->kfifo.buf_out = 0;
            uart->RXSADR = (u32)uart_bus->kfifo.buffer;
            uart->RXEADR = (u32)(uart_bus->kfifo.buffer + uart_bus->kfifo.buf_size);
        }

        uart->CON1	 =  UART_POWEROFF[i].CON1;
        /* uart->BAUD	 =  UART_POWEROFF[i].BAUD; */
        uart->BAUD	 = ((UART_CLK + uart_arg->baud / 2) / uart_arg->baud) / 4 - 1;
        uart->TXADR	 =  UART_POWEROFF[i].TXADR;
        uart->RXCNT  =  UART_POWEROFF[i].RXCNT;
        /* uart->OTCNT  =  UART_POWEROFF[i].OTCNT; */
        if (uart_bus->kfifo.buffer) {
            if (uart_bus->rx_timeout > 10) {
                uart->OTCNT = (uart_bus->rx_timeout / 10) * (UART_OT_CLK / 10) / 10;
            } else {
        ...
        uart->CON2	 =  UART_POWEROFF[i].CON2;
        uart->CON0   =  UART_POWEROFF[i].CON0;
        uart->CON0  |= (BIT(13) | BIT(12) | BIT(10));
    }
    return 0;
}

DEEPSLEEP_TARGET_REGISTER(uart_dev) = {
    .name   = "uart_dev",
    .enter  = uart_dev_enter_deepsleep,
    .exit   = uart_dev_exit_deepsleep,
};

#endif/* #if (TCFG_LOWPOWER_LOWPOWER_SEL == DEEP_SLEEP_EN) */
/*-----------------------------------------------------------*/
