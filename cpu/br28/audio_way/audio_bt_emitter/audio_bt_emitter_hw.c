...
#if PC_AUDIO_EMITTER
    if(__this->ts.run_points> (CONFIG_BT_TX_BUFF_SIZE/2)){
        __this->ts.run_points = (CONFIG_BT_TX_BUFF_SIZE/2);
    }
#endif  /* PC_AUDIO_EMITTER */
    local_irq_enable();
    // printf("t:%d, %d, o:%d, f:%d, p:%d \n", __this->ts.bt_cnt, offset_time, run_once, get_bredr_tx_remain_size(), __this->ts.run_points); 
    // printf(" p:%d \n",  __this->ts.run_points); 
    if (__this->irq_en) {
        __this->irq_en = 0;
