...
{
    memset(vbat_voltage_array_f98, 0, (PMU_CH_VALUE_ARRAY_SIZE + 1)*2);
}

void vbat_voltage_array_f98_uninit(void)
{
    memset(vbat_voltage_array, 0, (PMU_CH_VALUE_ARRAY_SIZE + 1)*2);
}

u32 adc_get_value_f98(void)
{
    y_printf("DDD-vbat_voltage_array_f98:");
    put_buf(vbat_voltage_array_f98,sizeof(vbat_voltage_array_f98));
    return adc_value_avg(vbat_voltage_array_f98);
}
...
void f98_vbat_to_array(u16 vbat_f98)
{
    ...
    memcpy(temp_buf, vbat_voltage_array_f98, (PMU_CH_VALUE_ARRAY_SIZE - 1)*2);
    memcpy(&vbat_voltage_array_f98[1], temp_buf, (PMU_CH_VALUE_ARRAY_SIZE - 1)*2);
    vbat_voltage_array_f98[0] = vbat_f98;
    r_printf("F98_new_avg_buf:");
    put_buf(vbat_voltage_array_f98, (PMU_CH_VALUE_ARRAY_SIZE+1)*2);
