...
*/
/*----------------------------------------------------------------------------*/
extern u32 output_to_src_task(void *priv, s16 *data, int len);
                                 ...
                                 struct audio_data_frame *out)
{
    ...
    if (dec->remain == 0) {
        ...
        //audio_dac_write(&default_dac, in->data,  in->data_len);
        output_to_src_task(get_usb_mic_src_hdl(), in->data, in->data_len);
        //usb_audio_ext_mic_write(in->data,in->data_len);
        //output_to_src_task(get_usb_mic_src_hdl(), NULL, in->data_len);
#endif
...
static u16 esco_audio_samplerate  = 16000;
void *get_usb_mic_src_hdl(void);
void set_esco_audio_samplerate(u16 sr)
{
    r_printf("%s---sr= %d\n",__FUNCTION__, sr);
    esco_audio_samplerate =  sr;
    user_resample_set_sr(get_usb_mic_src_hdl(), esco_audio_samplerate, 0);
}

u16 get_esco_audio_samplerate(void)
{
    r_printf("---%s---sr = %d\n",__func__, esco_audio_samplerate);
    return esco_audio_samplerate;
}
...
static int esco_dec_start()
{
...
#if TCFG_ESCO_NOISEGATE
    NoiseGateParam esco_noisegate_parm = {
        .attackTime = 300,
        .releaseTime = 5,
    ...
    dec->noisegate = 	open_noisegate(&esco_noisegate_parm);
#endif /*TCFG_ESCO_NOISEGATE*/
    ...
    set_esco_audio_samplerate(dec->dec.sample_rate);
    extern u8 bt_mic_input_from_external_audio;
    if(bt_mic_input_from_external_audio == 0){
