...
#include "bt/bt.h"
#include "device/usb/device/hid.h"
...
#if PC_MIC_INPUT_EXTERNAL_AUDIO
u8 pc_mic_input_from_external_audio = 1;
#else
u8 pc_mic_input_from_external_audio = 0;
#endif
...
extern int usb_audio_mic_open_ext(void *_info);
extern int usb_audio_mic_close_ext(void *arg);
extern void usb_audio_mic_set_gain_ext(int gain);
...
#if PC_AUDIO_EMITTER
u8 device_support_vol_sync = 0;
u8 get_device_support_vol_sync()
{
    return device_support_vol_sync;
}
...
static int uac_audio_start(void)
{
    ...
    os_time_dly(1);
    uac_get_cur_vol(0, &test_l_volume, &test_l_volume);
    printf("uac_vol_switchlllll == %d\n",test_l_volume);
    if(test_l_volume != l_volume){
        device_support_vol_sync = 1;
    }
    else{
        device_support_vol_sync = 0;
    }
...
static int hfp_ag_start_delay_timer = 0;
void hfp_ag_start_delay(void *p)
{
    hfp_ag_start_delay_timer = 0;
    __emitter_send_media_toggle(NULL,0);
    
    user_emitter_cmd_prepare(USER_CTRL_HFP_CALL_LAST_NO,0,NULL); //开启 HFPAG 功能
}
//void TIMER_500()
//{
//    bt_emitter_pp(1);
...
*/
/*----------------------------------------------------------------------------*/
char MIC_OPEN_FLAG = 0;
static int usb_device_event_handler(u8 event, int value)
{
    switch (event) {
    case USB_AUDIO_PLAY_OPEN:
        /*tone_play_stop();*/
        g_printf("USB_AUDIO_PLAY_OPEN\n");
        usb_audio_play_open((void *)value);
#if PC_AUDIO_EMITTER
        u8 ss = bt_emitter_pp(1);
        r_printf(">>>>>usb play open == %d\n",ss);
#endif  /* PC_AUDIO_EMITTER */
        break;
    case USB_AUDIO_PLAY_CLOSE:
        g_printf("USB_AUDIO_PLAY_CLOSE\n");
        usb_audio_play_close((void *)value);
        break;
    case USB_AUDIO_MIC_OPEN:
    	    	extern void __set_disable_sco_flag(bool flag);
        __set_disable_sco_flag(0);
        r_printf("---USB_AUDIO_MIC_OPEN audio_sbc_enc_is_work == %d\n",audio_sbc_enc_is_work());
        MIC_OPEN_FLAG = 1;
#if PC_MIC_INPUT_EXTERNAL_AUDIO
        if(pc_mic_input_from_external_audio){
            usb_audio_mic_open_ext((void *)value);
            //延时一下再开通话，主要是先等其他状态正常
            if(hfp_ag_start_delay_timer == 0){
                hfp_ag_start_delay_timer = sys_timeout_add(NULL, hfp_ag_start_delay, 1000);
            }
        }
        else{
    ...
    case USB_AUDIO_MIC_CLOSE:
        r_printf("USB_AUDIO_MIC_CLOSE\n");
#if PC_MIC_INPUT_EXTERNAL_AUDIO
        MIC_OPEN_FLAG = 0;
        if(hfp_ag_start_delay_timer){
            sys_timeout_del(hfp_ag_start_delay_timer);
            hfp_ag_start_delay_timer = 0;
        }
        if(is_bt_emitter()){//发射器才需要hangup
            user_emitter_cmd_prepare(USER_CTRL_HFP_CALL_HANGUP,0,NULL); //关闭 HFPAG 功能
        }
        usb_audio_mic_close_ext((void *)value);
