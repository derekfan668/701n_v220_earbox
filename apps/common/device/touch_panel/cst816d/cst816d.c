...
static void iic_clock_set_delay(int need_delay);
...
void cst816d_reset();
void cst816d_bootmode_reset()
{
    CST816S_RESET_H();
    printf("cst816d_bootmode_reset\n");
    ctp_delay_ms(10);
    CST816S_RESET_L();
    printf("cst816d_bootmode_reset\n");
    ctp_delay_ms(10);
...
{
    uint8_t lvalue;
    iic_clock_set_delay(300); // 进入升级时设置延时300
...
static void touch_event_handler(void *priv)
{
    ...
    cst816d_read(&mytouch_data);
    touch_down = !mytouch_data.state;
    touch_x = mytouch_data.point.x;
    touch_y = VK_Y - mytouch_data.point.y - 1;
...
static void iic_clock_set_delay(int need_delay)
{
    ...
    if (sysclk > (96 * 1000000L)) {
        soft_iic_real_delay[0] = need_delay;
    } else if (sysclk > (48 * 1000000L)) {
        soft_iic_real_delay[0] = need_delay / 2;
    } else if (sysclk > (24 * 1000000L)) {
        soft_iic_real_delay[0] = need_delay / 4;
    } else {
        soft_iic_real_delay[0] = need_delay / 8;
    }
...
void cst816d_init()
{
    ...
    // soft_iic_real_delay[0] = 300;//
    iic_clock_set_delay(50); // 进入工作模式时延时设置为50
    cst816d_reset();
    u8 nbuf[1];
