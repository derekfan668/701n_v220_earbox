...
#if PC_AUDIO_EMITTER
static u8 change_usb_mode = 0;
void set_change_usb_mode(u8 mode)
{
    g_printf("%s[line:%d, mode:%d]\n",__func__,__LINE__, mode);
    
    change_usb_mode = mode;
    
}
u8 get_change_usb_mode(void)
{
    return change_usb_mode;
}
#endif  /* PC_AUDIO_EMITTER */
void usb_start()
{

    printf("%s[line:%d, task:%s]\n",__func__,__LINE__, os_current_task());
...
#else
#if PC_AUDIO_EMITTER
    if(get_change_usb_mode()){
        usb_device_mode(usbfd, MASSSTORAGE_CLASS|SPEAKER_CLASS|MIC_CLASS|HID_CLASS);

    }else{
        usb_device_mode(usbfd,SPEAKER_CLASS|MIC_CLASS|HID_CLASS);
    }
#else 
...
void pc_restart(void)
{
    ...
    if (usb_otg_online(0) == SLAVE_MODE) {
        ...
        //如果是发射器，等蓝牙连上再开PC
        if(is_bt_emitter() == 0 || is_bt_connected()){
