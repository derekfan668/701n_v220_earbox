...
int audio_output_data_db_calc_simple(short *data, unsigned short len,unsigned char channels)
{
    ...
    if(1){
        average_value= (sum[0]+sum[1]+sum[2]+sum[3])/(channels*points);
        sum[0] = 0;
        sum[1] = 0;
        sum[2] = 0;
        sum[3] = 0;
...
u8 spk_stream_status = 0;
u8 last_spk_stream_status = 0;
u8 get_spk_stream_status()
{
    return spk_stream_status;
}

#define FADE_TIME  100//ms

int user_clear_uac_buf()
{
    printf("cuf_get_data_len == %d\n",cbuf_get_data_len(&uac_speaker->cbuf));
    int len =cbuf_get_data_len(&uac_speaker->cbuf);
    char zero_buf[2048];
    cbuf_read(&uac_speaker->cbuf, zero_buf, len-128);
    memset(zero_buf,0x00,2048);
    cbuf_write(&uac_speaker->cbuf, zero_buf, len-128);
   
    //cbuf_clear(&uac_speaker->cbuf);
}

u8 get_speaker_stream_is_open()
{
...
{
#if PC_AUDIO_EMITTER
    int energy = audio_output_data_db_calc_simple(obuf,len,1);
    if(energy){
        spk_stream_status = 1;
    }
    else{
    ...
    // static u16 timer = 0;
    // if(spk_stream_status != last_spk_stream_status){
    //     timer++;
    //     if(timer == FADE_TIME){
    //         if(spk_stream_status){
    //             bt_emitter_pp(1);
    //         }
    //         else{
    //             bt_emitter_pp(0);
    //         }
    //         last_spk_stream_status = spk_stream_status;
    //         timer = 0;
    //     }
    // }
    // else{
    ...
    if (speaker_stream_is_open) {
        ...
        int wlen = external_audio_input(get_ext_audio2sco_src_hdl(), obuf, len);
        if(wlen == 0){
            wlen = cbuf_write(&uac_speaker->cbuf, (void *)obuf, len);
        }
        else{
            return;
        }
