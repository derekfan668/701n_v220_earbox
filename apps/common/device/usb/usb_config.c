...
#if PC_AUDIO_EMITTER
#define     AUDIO_DMA_SIZE  (288+4)//256+192
#else
...
#if PC_AUDIO_EMITTER
static u8 usb_online_status = 0;
void set_usb_online_status(u8 status)
{
    usb_online_status = status;
}

u8 get_usb_online_status(void)
{
    return usb_online_status;
}
...
#endif
#if PC_AUDIO_EMITTER
        set_usb_online_status(0);
        g_printf("%s[line:%d]\n",__func__,__LINE__);
        extern void local_play_post_app_core_event(u8 cmd);
        local_play_post_app_core_event(3);
#endif  /* PC_AUDIO_EMITTER */
