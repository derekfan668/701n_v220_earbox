...
static u8 avr_bat_percent_val = 0xff;
...
#define BATTERY_CAPACITY  500.0 // 电池总容量，单位mAh
static float BATTERY_RESISTOR  =  0.360;   //电池内阻

#define BATTERY_MAX_I	+400.0	//最大充电电流
#define BATTERY_MIN_I	-200.0	//最大放电电流
#define BATTERY_INVAID_I		(50.0)	//电流校验失效电流
#define BATTERY_CHARGE_SMALL_I	(60.0)	//充电最小电流
...
4.100, 4.067, 4.033, 4.02, 4.013, 4.005,3.988, 3.962, 3.926, 3.893,
 3.871, 3.858,3.848, 3.836, 3.822, 3.803, 3.779,3.747,3.606, 3.50,
};
...
extern bool get_vbat_value_from_f98;
void vbat_calculate_loop() 
{
    ...
    if(get_vbat_value_from_f98) {
        current_vbat = (float)(adc_get_value_f98())/1000.0;
        // g_printf("DDD - 982 adc_get_value_f98(): %d", (adc_get_value_f98()));
        // g_printf("DDD - 982 current_vbat: %f", current_vbat);
        printf("is_sleep = %d F98实际检测出vbat值>>>>>>>>>>>> current_vbat = %f,current_soc = %.2f",is_sleep,current_vbat,current_soc);
    } else {
        current_vbat = (float)(adc_get_voltage(AD_CH_VBAT) * 4)/1000.0;
        // g_printf("DDD - 701 adc_get_voltage(AD_CH_VBAT): %d", (adc_get_voltage(AD_CH_VBAT) * 4));
        // g_printf("DDD - 701 current_vbat: %f", current_vbat);
        printf("is_sleep = %d 701实际检测出vbat值>>>>>>>>>>>> current_vbat = %f,current_soc = %.2f",is_sleep,current_vbat,current_soc);
    }
/*         y_printf("vbat_voltage_array ---------:");
#define PMU_CH_VALUE_ARRAY_SIZE   (16 + 1)
extern u16 vbat_voltage_array[PMU_CH_VALUE_ARRAY_SIZE];
        put_buf(vbat_voltage_array,sizeof(vbat_voltage_array)); */
    if(is_sleep>2000 && !LVCMP_DET_GET() && 0) { //长时间休眠且非充电状态直接校准 is_sleep累加作为休眠时间设置
        ...
        if (!get_vbat_value_from_f98) { // 放电状态        
    ...
    printf("Current SOC: %d, clid state:%d\n", avr_bat_percent_val, sbox_get_box_charging());
    // if (!sbox_get_box_charging()) { // 非充电状态下由701检测更新
        sbox_battery_box_set(avr_bat_percent_val);
    // }
    reak_ok =1;
...
{    
	...
	if(!get_charge_online_flag()) {
		...
		current_vbat = (float)(adc_get_voltage(AD_CH_VBAT) * 4)/1000.0;
		float I = predicted_current(current_vbat, current_soc);
		printf("hzh VBAT INIT I = %f", I);
		// if(abs(I) > BATTERY_INVAID_I){
		// 	printf(" hzh BATTERY_INVAID_I REREAD VBAT P!!!!!!!");
...
void check_power_on_voltage(void)
{
    ...
    while (1) {
        ...
        printf("vbat: %d\n", val);
        // 提高开机电压
        if ((val < 370) || adc_check_vbat_lowpower()) {
            low_power_cnt++;
            normal_power_cnt = 0;
            if (low_power_cnt > 10) {
