...
#if (SMBOX_MULTI_BLE_MASTER_NUMS && SMBOX_MULTI_BLE_EN)
...
//记录handle 使用
target_hdl_t target_handle[SUPPORT_MAX_CLIENT];
...
int client_write_without_respond_send(void *priv, u8 *data, u16 len);
int client_operation_send(u16 handle, u8 *data, u16 len, u8 att_op_type);
int ble_enable_new_dev_scan(void);
//---------------------------------------------------------------------------
static  client_conn_cfg_t *client_config = NULL;
extern void ble_connect_snyc_info(void);


//指定索索名字
static const u8 test_remoter_name1[] = "jl_earphone_hu";//
u8 custom_remote_addr[6] = {1,2,3,4,5,6};   //默认的连接mac地址
...
static const client_match_cfg_t match_dev01 = {
    ...
    .compare_data_len = sizeof(custom_remote_addr), //去结束符
    .compare_data = custom_remote_addr,
    .bonding_flag = 0,
};
#endif
...
#if BLE_CONNCT_EARPHONE

static int user_ble_scan_out_timer =0;

// static int user_ble_scan_out_timer =0;
void user_check_into_scan(void)
{
    static char local_cld_state =-1;
    y_printf("user_check_into_scan %d %d %d\n",box_info.earphone_state,box_info.box_clid_status,local_cld_state);
    if((!box_info.earphone_state) || scan_ctrl_en){
        
        bt_ble_scan_enable(0, 1);
        if(user_ble_scan_out_timer){
            user_ble_scan_out_timer=0;
            sys_timeout_del(user_ble_scan_out_timer);
        }
        user_ble_scan_out_timer = sys_timeout_add(0,smbox_bt_ble_scan_enable,BLE_POWERON_PAGE_TIME);
    }
   if(box_info.box_clid_status != local_cld_state){
        local_cld_state=box_info.box_clid_status;
        r_printf("local_cld_state: %d", local_cld_state);
        if(local_cld_state ==1){
            lv_exit_sleep();
        }else if(local_cld_state==2){
            lv_enter_sleep();
        }
   }
}

void user_check_scan_timeout(void)
{
    y_printf("user_check_scan_timeout\n");
    user_ble_scan_out_timer=0;
    u8 cur_dev_cid;
...
void user_set_box_mac(void)
{
    y_printf("user_modify_box_mac\n");
    int ret =syscfg_write(VM_EARPHONE_MAC,custom_remote_addr,6);
    // put_buf(buf,len);
    put_buf(custom_remote_addr,6);
    y_printf("user_modify_box_mac%d\n",ret);
}
void user_get_box_mac(u8 *buf)
{
    u8 test_addr[6] = {1,2,3,4,5,6};
    y_printf("user_get_box_mac\n");
    int ret = syscfg_read(VM_EARPHONE_MAC,buf,6);
    if(ret!=6){
        memcpy(buf,test_addr,6);
        // syscfg_write(VM_EARPHONE_MAC,test_addr,6);
    }
    put_buf(buf,6);
    // return buf;
}

void sbox_receive_earphone_mac(void)
{
    user_set_box_mac();
    if (box_info.earphone_state)
    {
...
static void ble_report_data_deal(att_data_report_t *report_data, target_uuid_t *search_uuid)
{
    ...
    case GATT_EVENT_NOTIFICATION:  //notify
        ...
        //  ble_notify_recv_data_handler(report_data->blob, report_data->blob_length);
        int ret = ble_recv_musicdata_handler(report_data->blob, report_data->blob_length);
        if(ret) {
            b_printf("return");
            // b_printf("report_data->blob_length: %d", report_data->blob_length);
            break;
        }
...
static bool check_device_is_match(u8 info_type, u8 *data, int size)
{
    ...
    for (i = 0; i < CLIENT_MATCH_CONN_MAX; i++) {
        ...
        //  y_printf("check_device_is_match %d %d %d %d\n",cfg->create_conn_mode,conn_mode,size,cfg->compare_data_len);
    ...
    case HCI_EVENT_PACKET:
        ...
        case HCI_EVENT_LE_META:
            ...
            case HCI_SUBEVENT_LE_CONNECTION_COMPLETE: {
                ...
                sys_timeout_add(NULL, ble_connect_snyc_info, 2000);
        ...
        case HCI_EVENT_DISCONNECTION_COMPLETE: {
            ...
            sbox_ble_connect_flag_set(0);
            sbox_bt_connect_flag_set(0);
            if (tmp_index == SMBOX_MULTI_INVAIL_INDEX) {
                log_info("unknown_handle:%04x\n", tmp_handle);sbox_ble_connect_flag_set(0);
            sbox_bt_connect_flag_set(0);
                break;
            }
            ...
            if (0) { //(conn_pair_info.pair_flag && SUPPORT_MAX_CLIENT < 2) {
                ...
                user_ble_scan_out_timer = sys_timeout_add(NULL,set_adv_close,BLE_RECONN_TIME);
#else
                if (packet[5] == 0x3e) {
...
#if BLE_POWER_ON_PAGE
    user_ble_scan_out_timer = sys_timeout_add(NULL,user_check_scan_timeout,BLE_POWERON_PAGE_TIME);
#endif
}
...
void smbox_bt_multi_client_init(void)
{
    log_info("%s\n", __FUNCTION__);
#if BLE_CONNCT_EARPHONE
    user_get_box_mac(custom_remote_addr);
#endif
...
void client_send_conn_param_update(void)
{
    ...
    printf("client update param:-%d-%d-%d-%d-\n", param->interval_min, param->interval_max, param->latency, param->timeout);
    if (client_con_handle[cur_dev_cid]) {
