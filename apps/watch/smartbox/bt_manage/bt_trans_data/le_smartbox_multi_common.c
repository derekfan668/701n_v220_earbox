...
#define CHANGE_TO_WATACH          1//对某些和_smartbox_module.c冲突实行屏蔽
...
void smbox_multi_ble_profile_init(u8 enable)
{
    ...
    if (enable) {
        ...
        if (config_le_sm_support_enable) {
            le_device_db_init();
#if PASSKEY_ENTER_ENABLE || (!TCFG_BLE_BRIDGE_EDR_ENALBE)
...
#if SUPPORT_MAX_SERVER
#if(SMBOX_MULTI_SEL_FUNC != SLAVE_CONN_AI_APP)
        server_profile_init();
#endif /* (SMBOX_MULTI_SEL_FUNC != SLAVE_CONN_AI_APP) */
#endif
