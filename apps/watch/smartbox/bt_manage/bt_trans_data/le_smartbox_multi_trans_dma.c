...
#include "le_smartbox_multi_trans_dma.h"
...
#if (SMBOX_MULTI_BLE_SLAVE_NUMS && SMBOX_MULTI_BLE_EN && (SMBOX_MULTI_SEL_FUNC == SLAVE_CONN_AI_APP))
...
#define log_info(x, ...)  printf("[LE-MUL-DMA]" x " ", ## __VA_ARGS__)
...
void smbox_trans_cbk_packet_handler(uint8_t packet_type, uint16_t channel, uint8_t *packet, uint16_t size)
{
    ...
    case HCI_EVENT_PACKET:
        ...
        case HCI_EVENT_DISCONNECTION_COMPLETE: {
            ...
            log_info("HCI_EVENT_DISCONNECTION_COMPLETE_TRANS: %0x---dma_module_en:%d\n", packet[5], dma_module_en);
...
#endif
    return app_send_user_data(server_con_handle[cur_dev_cid], ATT_CHARACTERISTIC_0000FF00_0000_1000_8000_00805F9B34FB_01_VALUE_HANDLE, data, len, ATT_OP_AUTO_READ_CCC);
}
...
#endif
    return app_send_user_data(server_con_handle[cur_dev_cid], ATT_CHARACTERISTIC_b84ac9c6_29c5_46d4_bba1_9d534784330f_01_VALUE_HANDLE, data, len, ATT_OP_AUTO_READ_CCC);
}
...
void dma_ble_adv_enable(u8 enable)
{
    ...
    if (dma_check_tws_is_master()) {
        ...
        smbox_multi_set_adv_enable(0, 0);
...
#endif
    return 0;//app_send_user_data(server_con_handle[cur_dev_cid], ATT_CHARACTERISTIC_ae02_02_VALUE_HANDLE, data, len, ATT_OP_AUTO_READ_CCC);
}
