...
LCD_SPI_PLATFORM_DATA_BEGIN(lcd_spi_data)
    ...
    .pin_dc = IO_PORTA_02,
    .pin_en = IO_PORTA_06,
    .pin_te = NO_CONFIG_PORT/* IO_PORTA_05 */,
#endif

#if TCFG_LCD_GC9307_172X320
    .pin_reset= IO_PORTA_04,
    .pin_cs	= IO_PORTA_03,
    .pin_bl = TCFG_BACKLIGHT_PWM_IO,
    .pin_dc	= IO_PORTA_02,
...
struct port_wakeup port5 = {
    ...
    .edge               = FALLING_EDGE, 
    // .filter             = PORT_FLT_16ms,                     //唤醒方式选择,可选：上升沿\下降沿
    .iomap              = UART_RX_982,                    //唤醒口选择
};
#if TCFG_CHARGE_ENABLE
...
void board_init()
{
	...
	// dacrp和pg8合绑，设置高阻
	gpio_set_pull_up(IO_PORTG_08, 0);
	gpio_set_pull_down(IO_PORTG_08, 0);
	gpio_set_direction(IO_PORTG_08, 1);
    gpio_set_die(IO_PORTG_08, 0);
    gpio_set_dieh(IO_PORTG_08, 0);
#endif /* #if (TCFG_AUDIO_DAC_CONNECT_MODE == DAC_OUTPUT_MONO_LR_DIFF) */

	// pc8和dm合绑，设置高阻
	gpio_set_pull_up(IO_PORTC_08, 0);
	gpio_set_pull_down(IO_PORTC_08, 0);
	gpio_set_direction(IO_PORTC_08, 1);
    gpio_set_die(IO_PORTC_08, 0);
    gpio_set_dieh(IO_PORTC_08, 0);

	// pa15和dp合绑，设置高阻
	gpio_set_pull_up(IO_PORTA_15, 0);
	gpio_set_pull_down(IO_PORTA_15, 0);
	gpio_set_direction(IO_PORTA_15, 1);
    gpio_set_die(IO_PORTA_15, 0);
    gpio_set_dieh(IO_PORTA_15, 0);
...
#if TCFG_PAY_ALIOS_ENABLE
#if (TCFG_PAY_ALIOS_WAY_SEL == TCFG_PAY_ALIOS_WAY_ALIYUN)
	extern void aliyun_upay_init(void);
	aliyun_upay_init();
...
void board_set_soft_poweroff(void)
{
...
#if 0//使用p11 传感器需要注意以下情况，关机前需要把特定传感器io 在主系统设置到目标的io状态，然后把io控制权恢复到系统，并且保护该io关机
    //test1 设置高阻
    gpio_set_pull_up(GPIO_1_TEST, 0);
    gpio_set_pull_down(GPIO_1_TEST, 0);
    gpio_set_direction(GPIO_1_TEST, 1);
    soff_gpio_protect(GPIO_1_TEST);

    //test2 设置输出0
    gpio_set_pull_up(GPIO_2_TEST, 0);
    gpio_set_pull_down(GPIO_2_TEST, 0);
    gpio_set_output_value(GPIO_2_TEST, 0);
    gpio_set_direction(GPIO_2_TEST, 0);
    soff_gpio_protect(GPIO_2_TEST);
    P11_PORT->PB_SEL = 0;//切换io 控制全到主系统
#endif
    ...
    // key_wakeup_enable(); // 关机时关闭按键唤醒，防止仓储模式按键唤醒，改为在开机使能按键唤醒
...
#if TCFG_RTC_CLOSE_DEMO
	// 使用时应该加上船运模式判断
    extern u8 storage_mode_off;
    if(storage_mode_off){
        M2P_LRC_KEEP = 1; // en lrc
        P3_OSL_CON |= (BIT(0));
        P3_OSL_CON |= (BIT(1)); // en 32k
    }else{
        M2P_LRC_KEEP = 0; // dis lrc
        P3_OSL_CON &= ~(BIT(0));
        P3_OSL_CON &= ~(BIT(1)); // dis 32k
    }
#endif
}
...
#endif /* #ifdef CONFIG_BOARD_7014F5_DEMO*/
