#ifndef CONFIG_BOARD_7014F5_DEMO_CFG_H
#define CONFIG_BOARD_7014F5_DEMO_CFG_H

#include "board_7014f5_demo_global_build_cfg.h"

#ifdef CONFIG_BOARD_7014F5_DEMO

#define TCFG_FUNC_TINY_ENABLE				0 // 功能精简。关闭UI、本地播放、蓝牙发射、宜动健康APP等相关功能
...
#define UART_TX_982                         IO_PORTB_02
#define UART_RX_982                         IO_PORTB_03           
...
#define TCFG_LC_FLASH_UPDATE_ENABLE   0 //内置flash升级使能
...
//内置FLASH虚拟文件系统空间大小
#if TCFG_LC_FLASH_UPDATE_ENABLE //内置flash升级使能
//内置flash升级 12.5m(0xc80000) - 128k  = 0xC60000
#define TCFG_VIRFAT_INSERT_FLASH_SIZE	  	(0xC80000-(!!TCFG_NOR_VM)*CONFIG_EXTERN_USER_VM_FLASH_SIZE)
#else
...
#define CONFIG_RULE_INSERT_FLASH_SIZE	  	0xC60000//0xE60000
#define CONFIG_RULE_INSERT_UI_VM_BASE	  	0xdde000//0xfde000 12.5m开始
#define CONFIG_RULE_INSERT_UI_VM_SIZE	  	0x20000//128k
#define CONFIG_RULE_LC_FLASH_UPDATE_BASE    0xdfe000 //内置flash升级起始地址
#define CONFIG_RULE_LC_FLASH_UPDATE_SIZE	0x180000 //ufw升级文件占的空间
#else
#define CONFIG_RULE_INSERT_FLASH_BASE	  	0x17e000 //1.5m - 8k开始 内置FLASH虚拟文件系统空间基础地址
...
#undef  TCFG_VIRFAT_INSERT_FLASH_ENABLE
#define TCFG_VIRFAT_INSERT_FLASH_ENABLE 	(!TCFG_FUNC_TINY_ENABLE)// 内置FLASH虚拟文件系统

#undef  CONFIG_EXTERN_FLASH_SIZE
#define CONFIG_EXTERN_FLASH_SIZE            0 //配置外挂FLASH大小

//内置FLASH虚拟文件系统空间大小
//6.0m(0x600000) - 128k  = 0x5e0000
#define TCFG_VIRFAT_INSERT_FLASH_SIZE	  	(0x600000-(!!TCFG_NOR_VM)*CONFIG_EXTERN_USER_VM_FLASH_SIZE)
#define TCFG_VIRFAT_INSERT_FLASH_BASE	  	0x1fe000 //2.0m - 8k开始 内置FLASH虚拟文件系统空间基础地址
//需要保留flash 最后8k 空间
...
//ini 工具文件使用 工具只能识别立即数 要和上面数据对上
#define CONFIG_RULE_INSERT_FLASH_BASE	  	0x1fe000 //2.0m - 8k开始 内置FLASH虚拟文件系统空间基础地址
#define CONFIG_RULE_INSERT_FLASH_SIZE	  	0x5e0000
#define CONFIG_RULE_INSERT_UI_VM_BASE	  	0x7de000 //(2.0m - 8k) + 6.0m - 128k开始
#define CONFIG_RULE_INSERT_UI_VM_SIZE	  	0x20000//128k
...
//                                  SD 配置                                        //
//*********************************************************************************//
#define TCFG_SD0_ENABLE						1//0//ENABLE_THIS_MOUDLE	//SD0模块使能
...
#define CONFIG_UI_STYLE                     STYLE_JL_NULL
#else
...
#define TCFG_TP_INT_IO                      IO_PORTC_07 //TP中断脚
#define TCFG_TP_RESET_IO                    IO_PORTC_03 //TP复位脚
#define TCFG_TP_POWER_IO                    IO_PORTC_06 //TP电源脚
...
#define TCFG_LCD_FLIP_ENABLE                DISABLE_THIS_MOUDLE //显示翻转180度
...
#define TCFG_BACKLIGHT_PWM_IO               IO_PORTC_01
...
#define TCFG_UI_ENABLE_STANDBY_DIAL			(TCFG_UI_ENABLE && ENABLE) // 表盘浏览
#define TCFG_UI_ENABLE_PULLDOWN_MENU		(TCFG_UI_ENABLE && ENABLE) // 下拉菜单
#define TCFG_UI_ENABLE_LEFT_MENU            (TCFG_UI_ENABLE && DISABLE) // 左滑侧边栏,ui_page_manager.h把SIDEBAR_LEFT_MENU_ENABLE置1关闭卡片管理表盘右滑功能
#define TCFG_UI_ENABLE_SYS_SET              (TCFG_UI_ENABLE && ENABLE)

// 需要使能TCFG_APP_RTC_EN
#define TCFG_UI_ENABLE_ALARM				(TCFG_UI_ENABLE && ENABLE) // 闹钟功能
#define TCFG_UI_ENABLE_STOPWATCH			(TCFG_UI_ENABLE && ENABLE) // 计时器
#define TCFG_UI_ENABLE_TIMER_ACTION			(TCFG_UI_ENABLE && ENABLE) // 倒计时

// 需要使能TCFG_APP_PC_EN
#define TCFG_UI_ENABLE_PC					(TCFG_UI_ENABLE && ENABLE) // PC模式

// 需要使能TCFG_APP_RECORD_EN
#define TCFG_UI_ENABLE_RECORD				(TCFG_UI_ENABLE && ENABLE) // RECORD模式

// 需要使能TCFG_APP_MUSIC_EN
#define TCFG_UI_ENABLE_MUSIC				(TCFG_UI_ENABLE && ENABLE) // 音乐
#define TCFG_UI_ENABLE_MUSIC_MENU			(TCFG_UI_ENABLE && ENABLE) // 音乐菜单
#define TCFG_UI_ENABLE_FILE					(TCFG_UI_ENABLE && ENABLE) // 文件浏览

// 需要使能TCFG_USER_EMITTER_ENABLE
#define TCFG_UI_ENABLE_BT_PAGE_ACTION		(TCFG_UI_ENABLE && ENABLE) // 蓝牙耳机列表
#define TCFG_UI_ENABLE_BT_SCAN				(TCFG_UI_ENABLE && ENABLE) // 搜索蓝牙耳机
#define TCFG_UI_ENABLE_PAGE_TOUCH			(TCFG_UI_ENABLE && ENABLE) // 已保存蓝牙耳机列表
#define TCFG_UI_ENABLE_SCAN_TOUCH			(TCFG_UI_ENABLE && ENABLE) // 搜索蓝牙耳机

#define TCFG_UI_ENABLE_PHONE_ACTION			(TCFG_UI_ENABLE && ENABLE) // 蓝牙通话
#define TCFG_UI_ENABLE_PHONEBOOK			(TCFG_UI_ENABLE && ENABLE) // 电话本
#define TCFG_UI_ENABLE_FINDPHONE			(TCFG_UI_ENABLE && ENABLE) // 找手机
#define TCFG_UI_ENABLE_NOTICE				(TCFG_UI_ENABLE && ENABLE) // 手机消息通知
#define TCFG_UI_ENABLE_WEATHER				(TCFG_UI_ENABLE && ENABLE) // 天气

#define TCFG_UI_ENABLE_FLASHLIGHT			(TCFG_UI_ENABLE && ENABLE) // 手电筒
#define TCFG_UI_ENABLE_QR_CODE				(TCFG_UI_ENABLE && ENABLE) // 二维码

#define TCFG_UI_ENABLE_CALENDAR				(TCFG_UI_ENABLE && ENABLE) // 日历
#define TCFG_UI_ENABLE_CALCULATOR			(TCFG_UI_ENABLE && ENABLE) // 计算器

// 需要使能TCFG_BMP280_ENABLE
#define TCFG_UI_ENABLE_ALTIMETER			(TCFG_UI_ENABLE && ENABLE) // 海拔高度
#define TCFG_UI_ENABLE_PRESSURE				(TCFG_UI_ENABLE && ENABLE) // 海拔气压

// 需要使能TCFG_GSENSOR_ENABLE、TCFG_HR_SENSOR_ENABLE
#define TCFG_UI_ENABLE_SPORT_INFO			(TCFG_UI_ENABLE && ENABLE) // 运动状态
#define TCFG_UI_ENABLE_SPORTRECORD			(TCFG_UI_ENABLE && ENABLE) // 运动记录
#define TCFG_UI_ENABLE_TRAIN				(TCFG_UI_ENABLE && ENABLE) // 锻炼
#define TCFG_UI_ENABLE_SPORTRECORD_BROWSE	(TCFG_UI_ENABLE && ENABLE) // 运动记录浏览
#define TCFG_UI_ENABLE_OXYGEN				(TCFG_UI_ENABLE && ENABLE) // 血氧饱和度
#define TCFG_UI_ENABLE_HEART				(TCFG_UI_ENABLE && ENABLE) // 心率
#define TCFG_UI_ENABLE_BREATH_TRAIN			(TCFG_UI_ENABLE && ENABLE) // 呼吸训练
#define TCFG_UI_ENABLE_HEAT					(TCFG_UI_ENABLE && ENABLE) // 压力
#define TCFG_UI_ENABLE_SLEEP				(TCFG_UI_ENABLE && ENABLE) // 睡眠

#define TCFG_UI_ENABLE_ALIPAY_CODE			(TCFG_UI_ENABLE && ENABLE) // 支付宝
#define TCFG_UI_ENABLE_DEMO                 (TCFG_UI_ENABLE && DISABLE) //示例
...
#endif //CONFIG_BOARD_7014F5_DEMO
#endif //CONFIG_BOARD_7014F5_DEMO_CFG_H
