...
void set_poweron_idle(u8 idle)
{
    poweron_idle = idle;
}
int get_poweron_idle()
{
    return poweron_idle;
}
//映射id
...
void app_poweron_task()
{
    ...
    int err =  true; // tone_play_with_callback_by_name((char *)tone_table[IDEX_TONE_POWER_ON], 1, tone_play_end_callback, (void *)IDEX_TONE_POWER_ON);
