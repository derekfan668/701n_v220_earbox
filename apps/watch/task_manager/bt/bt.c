...
extern void local_play_bt_status_handler(const char *type, u32 arg);
...
void bredr_handle_register()
{
#if ((APP_ONLINE_DEBUG) && (TCFG_COMM_TYPE == TCFG_SPP_COMM))
...
static int bt_connction_status_event_handler(struct bt_event *bt)
{
    ...
    case BT_STATUS_INIT_OK:
        ...
        if (pc_app_check() == true) {
            printf("app_task_switch_to(APP_PC_TASK)\n");
...
#if PC_AUDIO_EMITTER
        g_printf("__emitter_send_media_toggle\n");
        PHONE_OPEN_FLAG = 1;
#endif  /* PC_AUDIO_EMITTER */
        //当手机app与手表传输过程中不能响应通话事件
        if (__this->smartbox_watch_upgrade_flag) {
...
#if PC_AUDIO_EMITTER
        bt_a2dp_flag = 1;
        printf("[BT CONNECT 2222222222222222222222222222222222222]\n");
//固定最大音量  连接后发送一次最大音量
        // bt_emitter_set_vol(30);
        // os_time_dly(1);
/////////
		if (pc_app_check() == true) {
            printf("%s[line:%d]\n",__func__,__LINE__);
...
static int bt_hci_event_handler(struct bt_event *bt)
{
    ...
    local_play_bt_status_handler("hci_event", bt->event);
    if (bt_hci_event_filter(bt) == 0) {
        return 0;
    }
    ...
    case HCI_EVENT_DISCONNECTION_COMPLETE :
        ...
        if (bt->value == ERROR_CODE_PAIRING_NOT_ALLOWED) { //用于判断华为手表的断开情况
            ...
            local_play_bt_status_handler("hci_value", bt->value);
        }
        bt_hci_event_disconnect(bt) ;
        clock_remove_set(BT_CONN_CLK);
        break;
...
int bt_background_event_handler_filter(struct sys_event *event)
{
    ...
    if ((u32)event->arg == SYS_BT_EVENT_TYPE_CON_STATUS) {
        ...
        if (!send) {
            ...
            local_play_bt_status_handler("event", event->u.bt.event);
        }
    } else if ((u32)event->arg == SYS_BT_EVENT_TYPE_HCI_STATUS) {
...
#if PC_AUDIO_EMITTER
    case  KEY_PC_VOL_CON:
        log_info("    KEY_PC_VOL_UP \n");
        int vol = 0;
        int i = 0;
        vol = get_control_pc_vol_time();
        if(vol > 0){
            for(i = 0;i<vol;i++){
                printf("hid_up\n");
                hid_key_handler(0, USB_AUDIO_VOLUP);
                add_control_pc_vol_time(-1);
            }
        }
        else if(vol < 0){
            for(i = 0;i > vol;i--){
