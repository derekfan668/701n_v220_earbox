...
void bt_check_enter_sniff()
{
    ...
    extern u8 transitcode_task_idle_query(void);
    if (!transitcode_task_idle_query()) {
        return;
    }
#endif
#endif
...
void sys_auto_sniff_controle(u8 enable, u8 *addr)
{
    ...
    return;
#endif
    printf("sys_auto_sniff_controle == %d\n",enable);
...
#if SMBOX_MULTI_BLE_SLAVE_NUMS
    extern u16 bt_ble_is_connected(void);
    if (bt_ble_is_connected()) {
        return;
    }
#endif /* SMBOX_MULTI_BLE_SLAVE_NUMS */
#endif
...
u8 bt_status_event_filter(struct bt_event *bt)
{
    ...
    local_play_bt_status_handler("event", bt->event);
    return true;
}
...
void bt_status_phone_out(struct bt_event *bt)
{
    ...
    user_send_cmd_prepare(USER_CTRL_HFP_CALL_CURRENT, 0, NULL); //发命令获取电话号码
#if PC_AUDIO_EMITTER
    user_emitter_cmd_prepare(USER_CTRL_HFP_CALL_ANSWER, 0, NULL); //发射器发起通话需要直通
...
u8 user_bt_sniff_flag = 0;
u8 bt_is_sniff_close(void)
{
    return user_bt_sniff_flag;
}
/*----------------------------------------------------------------------------*/
...
void bt_status_sniff_state_update(struct bt_event *bt)
{
    ...
    if (bt->value == 0) {
        ...
        user_bt_sniff_flag = 0;
    } else {
#if (TCFG_USER_TWS_ENABLE == 1)
        pwm_led_clk_set(PWM_LED_CLK_RC32K);
#endif
        sys_auto_sniff_controle(0, bt->args);
        user_bt_sniff_flag = 1;
    }
}
