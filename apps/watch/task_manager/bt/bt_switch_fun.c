...
static void a2dp_media_packet_play_start(void *p)
{
    ...
    if ((__this->exit_flag == 0) || (__this->sbc_packet_step == 2)) {
        log_debug(" a2dp back \n");
#if TCFG_MUSIC_IDLE_ENABLE_BACK
        if (!bt_get_a2dp_en_status()) {
            bt_set_a2dp_en_status(1);
            bt_set_music_device_style(SET_MUSIC_IN_PHONE);
            UI_MSG_POST("music_start:dev_switch=%4", SET_MUSIC_IN_PHONE);
        }
#endif
...
#if TCFG_APP_MUSIC_EN && TCFG_MUSIC_IDLE_ENABLE_BACK
    if (cur_task == APP_MUSIC_TASK) {
        if (music_player_get_play_status() == FILE_DEC_STATUS_PLAY) {
            // 播放过程中不容许被切回，其他状态容许切回
            local_irq_enable();
            return -EINVAL;
        }
    } else
