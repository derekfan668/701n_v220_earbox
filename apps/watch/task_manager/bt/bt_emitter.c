...
#if 0//!TCFG_USER_BLE_CTRL_BREDR_EN
...
u8 search_bd_name_filt(char *data, u8 len, u32 dev_class, char rssi)
{
    ...
    log_info(" edr name:%s,len:%d,class %x ,rssi %d\n", bd_name, len, dev_class, rssi);
#if 0
...
#if PC_AUDIO_EMITTER
        if(extern_app_ctrl_pc_key(KEY_MUSIC_NEXT)){
            break;
        }
...
#if PC_AUDIO_EMITTER
        if(extern_app_ctrl_pc_key(KEY_MUSIC_PREV)){
            break;
        }
#endif  /* PC_AUDIO_EMITTER */
...
#if PC_AUDIO_EMITTER
        if(extern_app_ctrl_pc_key(KEY_MUSIC_PP)){
            break;
        }
#endif  /* PC_AUDIO_EMITTER */
#if TCFG_APP_RTC_EN
...
#if PC_AUDIO_EMITTER
        if(extern_app_ctrl_pc_key(KEY_VOL_UP)){
            break;
        }
#endif  /* PC_AUDIO_EMITTER */
...
#if PC_AUDIO_EMITTER
        if(extern_app_ctrl_pc_key(KEY_VOL_DOWN)){
            break;
        }
#endif  /* PC_AUDIO_EMITTER */
...
u8 donot_sync_vol = 0;
u8 get_donot_sync_vol()
{
    printf("donot_sync_vol == %d\n",donot_sync_vol);
    return donot_sync_vol;
}
void dec_donot_sync_vol()
{
    if(donot_sync_vol > 0){
        donot_sync_vol -= 1;
    }
...
extern void emeet_control_from_receive(u8 vol);
/*----------------------------------------------------------------------------*/
/**@brief    蓝牙发射接收设备同步音量
   @param    vol:接收到设备同步音量
   @return   无
...
#if PC_AUDIO_EMITTER
    emeet_control_from_receive(vol);
#endif  /* PC_AUDIO_EMITTER */
}
...
{
    g_printf("audio_sbc_enc_open_exit\n");
...
#endif
#if PC_AUDIO_EMITTER
u8 is_bt_emitter(void)
{
    return bt_user_priv_var.emitter_or_receiver == BT_EMITTER_EN ? 1 : 0;
}

u8 is_bt_connected(void)
{
    u16 (*curr_channel_state)(void) = bt_user_priv_var.emitter_or_receiver == BT_EMITTER_EN ? get_emitter_curr_channel_state : get_curr_channel_state;
    return curr_channel_state() ? 1 : 0;
}

extern u8 bt_mic_input_from_external_audio;//bt是否使用本地mic， 0使用，1不使用
extern u8 pc_mic_input_from_external_audio;//pc是否使用本地mic， 0使用，1不使用

void set_external_audio_input_en(u8 en)
{
    bt_mic_input_from_external_audio = en;
    pc_mic_input_from_external_audio = en;
}
#endif  /* PC_AUDIO_EMITTER */
