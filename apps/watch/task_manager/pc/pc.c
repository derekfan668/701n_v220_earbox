...
extern u8 set_user_mute_emitter(u8 mute);
int mute_emitter_flag = 0;
void resume_emitter_data()
{
    printf("resume_emitter_data\n");
    set_user_mute_emitter(0);
    mute_emitter_flag = 0;
}
...
void open_music()
{
    music_pp_flag = 0;
    printf("open_music\n");
    if(mute_emitter_flag == 0){
        set_user_mute_emitter(1);
        //user_clear_uac_buf();
        mute_emitter_flag = sys_timeout_add(NULL,resume_emitter_data,500);
    }
    bt_emitter_pp(1);    
}

int extern_app_ctrl_pc_key(int event)
{
    int ret = true;
        ...
        //(r == SLAVE_MODE_WAIT_CONFIRMATION))
    {
        ...
        case  KEY_MUSIC_PP:
            ...
            hid_key_handler(0, USB_AUDIO_PP);//USB_AUDIO_PAUSE);//USB_AUDIO_PP);
            break;
        case  KEY_MUSIC_PREV:
            log_info("KEY_MUSIC_PREV\n");
            hid_key_handler(0, USB_AUDIO_PREFILE);
