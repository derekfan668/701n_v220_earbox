...
const int CONFIG_BT_TX_BUFF_MAX_SIZE         = (20 * 1024);
...
#else
#if SMBOX_MULTI_BLE_EN
const int config_btctler_le_hw_nums = (CONFIG_BT_GATT_CONNECTION_NUM + 1);//watch设置主从机个数
#else
...
// LE RAM Control
const int config_btctler_le_rx_nums = ((CONFIG_BT_GATT_CONNECTION_NUM + 1) * 3) + 4;
const int config_btctler_le_acl_packet_length = 251;
const int config_btctler_le_acl_total_nums = ((CONFIG_BT_GATT_CONNECTION_NUM+1) * 3) + 4;//watc
#else
