...
/* ------------------------------------------------------------------------------------*/
/**
 * @brief lcd_write_aod_ram 写屏幕AOD RAM接口
 *
 * @param cmd : 0x4c 或 0x5c
 * @param buf : 待发送buf地址
 * @param len : 待发送buf长度（需要按100对齐，以100字节为单位）
 *
 * @return 0
 */
/* ------------------------------------------------------------------------------------*/
void lcd_write_aod_ram(u32 cmd, u8 *buf, u32 len);

int lcd_draw_area_reversed(int lcd_width, int lcd_height, u8 *lcd_buf, int left, int top, int width, int height, int wait);
