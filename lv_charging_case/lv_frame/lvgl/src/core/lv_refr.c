...
static void refr_invalid_areas(void)
{
    ...
    for(i = 0; i < disp_refr->inv_p; i++) {
        ...
        if(disp_refr->inv_area_joined[i] == 0) {
            ...
            // lv_area_t ori = disp_refr->inv_areas[i];

            // if(disp_refr->inv_areas[i].x2 > 160){
            //     disp_refr->inv_areas[i].x1 = LV_MAX(disp_refr->inv_areas[i].x1, 160);
            //     refr_area(&disp_refr->inv_areas[i]);
            // }
            // disp_refr->inv_areas[i] = ori;

            // if(disp_refr->inv_areas[i].x1 < 159){
            //     disp_refr->inv_areas[i].x2 = LV_MIN(disp_refr->inv_areas[i].x2, 159);
            //     refr_area(&disp_refr->inv_areas[i]);
            // }
            // disp_refr->inv_areas[i] = ori;
