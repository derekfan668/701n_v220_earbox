...
void lv_draw_sw_init_ctx(lv_disp_drv_t * drv, lv_draw_ctx_t * draw_ctx)
{
    ...
    draw_sw_ctx->base_draw.draw_arc = lv_draw_sw_arc;   //  进度条
    draw_sw_ctx->base_draw.draw_rect = lv_draw_sw_rect; //  框框
    draw_sw_ctx->base_draw.draw_bg = lv_draw_sw_bg;     //  背景
    draw_sw_ctx->base_draw.draw_letter = lv_draw_sw_letter; //文字
    draw_sw_ctx->base_draw.draw_img_decoded = lv_draw_sw_img_decoded;
    draw_sw_ctx->base_draw.draw_line = lv_draw_sw_line; //  线
    draw_sw_ctx->base_draw.draw_polygon = lv_draw_sw_polygon;   //  多边形
#if LV_DRAW_COMPLEX
    draw_sw_ctx->base_draw.draw_transform = lv_draw_sw_transform;   //  缩放
#endif
