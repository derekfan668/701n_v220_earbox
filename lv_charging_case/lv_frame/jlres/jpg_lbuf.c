...
static int _lbuf_free(void *lbuf);

struct lbuff_head *jpg_lbuf_init(void *buf, u32 len, int align, int priv_head_len)
{
    struct hfree *free;
    //首地址4对齐
    ...
    //printf("0000000000000000lbuf_init: %p,%x\n", head, (u32)head + len);

    return head;
}

int jpg_lbuf_avaliable(struct lbuff_head *head, int size)
{
    struct hfree *p;
    int avaliable = 0;
...
void jpg_lbuf_state(struct lbuff_head *head, struct lbuff_state *state)
{
...
void jpg_lbuf_dump(struct lbuff_head *head)
{
    int total_size = 0;
    struct hfree *p, *n;
    struct hentry *h;
    ...
    list_for_each_entry(h, &head->head, entry) {
        /* printf("alloc: %x, %x, call_from: %x\n", h, h->len, h->rets); */
    }

    log_i("lbuf_state:%x,%x\n", head->total_size, total_size);

    spin_unlock(&head->lock);
}

u32 jpg_lbuf_free_space(struct lbuff_head *head)
{
    int max_len = 0;
    struct hfree *p;
...
void *jpg_lbuf_alloc(struct lbuff_head *head, u32 len)
{
    int offset;
    int max_len = 0;
    void *ret = NULL;
...
int jpg_lbuf_remain_space(struct lbuff_head *head)
{
    int max_len = 0;
    struct hfree *p;
...
void *jpg_lbuf_realloc(void *lbuf, int size)
{
    int len;
    int offset;
    int head_len;
...
int jpg_lbuf_empty(struct lbuff_head *head)
{
...
void jpg_lbuf_clear(struct lbuff_head *head)
{
    struct hentry *p, *n;
...
int jpg_lbuf_real_size(void *lbuf)
{
    struct hentry *entry = __get_entry(lbuf);
...
void jpg_lbuf_inc_ref(void *lbuf)
{
    struct hentry *entry = __get_entry(lbuf);
...
void jpg_lbuf_push(void *lbuf, u8 channel_map)
{
    int i;
    //根据hentry结构体大小得到hentry结构体地址
...
int jpg_lbuf_traversal(struct lbuff_head *head)
{
    struct hentry *p;
    int num = 0;
...
void *jpg_lbuf_pop(struct lbuff_head *head, u8 channel)
{
    struct hentry *p;
...
void jpg_lbuf_free_check(void *lbuf, u32 rets)
{
    struct hentry *entry;
...
/* --------------------------------------------------------------------------*/
/**
 * @brief lbuf free: 此函数没添加临界区保护，仅本文件内部调用，调用前需保护
 *
 * @param lbuf lbuf
 *
 * @return ret
