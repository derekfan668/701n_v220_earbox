...
void path_exit(char *dst_path)
{
    ...
    /* printf("last_slash: %s, %x, %d", last_slash, last_slash, last_slash - dst_path); */
    if (last_slash == NULL)
    {
        printf("last_slash is null");
        return;
    }
    ...
    *(last_slash) = '\0';
    ...
    /* printf("last_slash: %s", last_slash); */

    second_last_slash = strrchr(dst_path, '/');

    if (second_last_slash == NULL)
    {
        printf("second_last_slash is null");
        return;
    }
...
char *get_music_file_title_by_id(int id)
{
    if (id >= sizeof(music_files) / sizeof(music_files[0])) {
        printf("id err, func: %s, line: %d", __func__, __LINE__);
        return NULL;
    }
...
static void file_free()
{
    for(int i = 0; i < MAX_FILE_NUM; i++) {
        if(music_files[i].name != NULL) {
            p_printf("free id: %d, addr: 0x%x", i, music_files[i].name);
...
static void show_file_info(const char *path, const char *arg)
{
    ...
    for(int i = 0; i < file_num; i++) {
        printf("id: %d, name: %s, clust: %d, dir_type: %d", music_files[i].index, music_files[i].name, music_files[i].clust, music_files[i].dir_type);
    }
    fscan_release(fs);
    fs = NULL;
}
...
static int open_file_handler(const char *path, const char *arg)
{
    ...
    if(fs == NULL)
    {
        printf("fs is null, please check the path");
        return -1 ;
    }
    ...
    if (fs->file_number == 0) { //判断是否有被扫描格式文件存在
        printf("there is no file in this path");
        fscan_release(fs);
        fs = NULL;
        return 0 ;
    }
    ...
    for(int i = 0; i < fs->file_number; i++)
    {
        ...
        if(fp)
        {
            ...
            if(music_files[i].name == NULL) {
                ...
                n_printf("malloc id: %d, addr: 0x%x", i, music_files[i].name);
            }

            // 申请不到内存
            if(music_files[i].name == NULL) {
                mem_stats();
                printf("id: %d malloc fail!");
                printf("real file num is %d", file_num);
                file_num = i + 1;
                printf("return file num is %d", file_num);
                break;
            }
