...
static void PageManagerInit(void)
{
	...
	// 初始化上下状态栏
    lv_status_bar_top_init(PM_GetBackGroudObj());       
    lv_status_bar_bottom_init(PM_GetBackGroudObj());

    // 创建页面关系
    PM_AddPage(&screen_music, init_screen_music, deinit_screen_music, ID_SCREEN_MUSIC);
    PM_AddPage(&screen_volume, init_screen_volume, deinit_screen_volume, ID_SCREEN_VOLUME);
    PM_AddPage(&screen_anc, init_screen_anc, deinit_screen_anc, ID_SCREEN_ANC);
    PM_AddPage(&screen_equalizer, init_screen_equalizer, deinit_screen_equalizer, ID_SCREEN_EQUALIZER);
    PM_AddPage(&screen_alarm, init_screen_alarm, deinit_screen_alarm, ID_SCREEN_ALARM);
    PM_AddPage(&screen_brightness, init_screen_brightness, deinit_screen_brightness, ID_SCREEN_BRIGHTNESS);
    PM_AddPage(&screen_language, init_screen_language, deinit_screen_language, ID_SCREEN_LANGUAGE);
    PM_AddPage(&screen_wallpaper, init_screen_wallpaper, deinit_screen_wallpaper, ID_SCREEN_WALLPAPER);
    PM_AddPage(&screen_findmy, init_screen_findmy, deinit_screen_findmy, ID_SCREEN_FINDMY);
    PM_AddPage(&screen_light, init_screen_light, deinit_screen_light, ID_SCREEN_LIGHT);
    PM_AddPage(&screen_time, init_screen_time, deinit_screen_time, ID_SCREEN_TIME);
    PM_AddPage(&screen_tiktok, init_screen_tiktok, deinit_screen_tiktok, ID_SCREEN_TIKTOK);
    PM_AddPage(&screen_snap, init_screen_snap, deinit_screen_snap, ID_SCREEN_SNAP);
    PM_AddPage(&screen_pc_mode, init_screen_pc_mode, deinit_screen_pc_mode, ID_SCREEN_PRODUCT_TEST);

    // screen_snap <- screen_music -> screen_volume
    //                      |
    //                screen_pc_mode
    PM_SetPageMoveMode(&screen_music, PageLeft, &screen_snap, LOAD_ANIM_MOVE);
    PM_SetPageMoveMode(&screen_music, PageRight, &screen_volume, LOAD_ANIM_MOVE);
    ...
    PM_SetPageMoveMode(&screen_volume, PageLeft, &screen_music, LOAD_ANIM_MOVE);
    PM_SetPageMoveMode(&screen_volume, PageRight, &screen_anc, LOAD_ANIM_MOVE);

    // screen_volume <- screen_anc -> screen_equalizer
    PM_SetPageMoveMode(&screen_anc, PageLeft, &screen_volume, LOAD_ANIM_MOVE);
    PM_SetPageMoveMode(&screen_anc, PageRight, &screen_equalizer, LOAD_ANIM_MOVE);

    // screen_anc <- screen_equalizer -> screen_alarm
    PM_SetPageMoveMode(&screen_equalizer, PageLeft, &screen_anc, LOAD_ANIM_MOVE);
    PM_SetPageMoveMode(&screen_equalizer, PageRight, &screen_alarm, LOAD_ANIM_MOVE);
    
    // screen_equalizer <- screen_alarm -> screen_brightness
    PM_SetPageMoveMode(&screen_alarm, PageLeft, &screen_equalizer, LOAD_ANIM_MOVE);
    PM_SetPageMoveMode(&screen_alarm, PageRight, &screen_brightness, LOAD_ANIM_MOVE);

    // screen_alarm <- screen_brightness -> screen_language
    PM_SetPageMoveMode(&screen_brightness, PageLeft, &screen_alarm, LOAD_ANIM_MOVE);
    PM_SetPageMoveMode(&screen_brightness, PageRight, &screen_language, LOAD_ANIM_MOVE);

    // screen_brightness <- screen_language -> screen_wallpaper
    PM_SetPageMoveMode(&screen_language, PageLeft, &screen_brightness, LOAD_ANIM_MOVE);
    PM_SetPageMoveMode(&screen_language, PageRight, &screen_wallpaper, LOAD_ANIM_MOVE);

    // screen_language <- screen_wallpaper -> screen_findmy
    PM_SetPageMoveMode(&screen_wallpaper, PageLeft, &screen_language, LOAD_ANIM_MOVE);
    PM_SetPageMoveMode(&screen_wallpaper, PageRight, &screen_findmy, LOAD_ANIM_MOVE);

    // screen_wallpaper <- screen_findmy -> screen_light
    PM_SetPageMoveMode(&screen_findmy, PageLeft, &screen_wallpaper, LOAD_ANIM_MOVE);
    PM_SetPageMoveMode(&screen_findmy, PageRight, &screen_light, LOAD_ANIM_MOVE);

    // screen_findmy <- screen_light -> screen_time
    PM_SetPageMoveMode(&screen_light, PageLeft, &screen_findmy, LOAD_ANIM_MOVE);
    PM_SetPageMoveMode(&screen_light, PageRight, &screen_time, LOAD_ANIM_MOVE);

    // screen_light <- screen_time -> screen_tiktok
    PM_SetPageMoveMode(&screen_time, PageLeft, &screen_light, LOAD_ANIM_MOVE);
    PM_SetPageMoveMode(&screen_time, PageRight, &screen_tiktok, LOAD_ANIM_MOVE);

    // screen_time <- screen_tiktok -> screen_snap
    PM_SetPageMoveMode(&screen_tiktok, PageLeft, &screen_time, LOAD_ANIM_MOVE);
    PM_SetPageMoveMode(&screen_tiktok, PageRight, &screen_snap, LOAD_ANIM_MOVE);

    // screen_tiktok <- screen_snap -> screen_music
    PM_SetPageMoveMode(&screen_snap, PageLeft, &screen_tiktok, LOAD_ANIM_MOVE);
    PM_SetPageMoveMode(&screen_snap, PageRight, &screen_music, LOAD_ANIM_MOVE);
