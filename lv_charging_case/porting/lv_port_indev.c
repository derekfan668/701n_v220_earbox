...
static void touchpad_read(lv_indev_drv_t *indev_drv, lv_indev_data_t *data)
{
    ...
    // p_printf("func: %s, line: %d", __func__, __LINE__);
...
static void touchpad_get_xy(lv_coord_t *x, lv_coord_t *y)
{
    ...
    if(touch_x<0)                   (*x) = 0;
    else if(touch_x>info.width)     (*x) = info.width;
    else if(touch_y<0)              (*y) = 0;
    else if(touch_y>info.height)    (*y) = info.height;
    else{
        (*x) = touch_x;
        (*y) = touch_y;        
    }
    // p_printf("x: %d, y: %d", touch_x, touch_y);
}
